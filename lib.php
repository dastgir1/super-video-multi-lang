...
function supervideo_add_instance(stdClass $supervideo, $mform = null) {
    ...
    \mod_supervideo\media\media_manager::save_from_form($supervideo, $supervideo);
...
function supervideo_delete_instance($id) {
    ...
    if ($cm) {
        ...
        \mod_supervideo\media\media_manager::delete_all($supervideo->id, $context);
...
function supervideo_get_coursemodule_info($coursemodule) {
    ...
    if (!$supervideo = $DB->get_record("supervideo", ["id" => $coursemodule->instance],
        "id, name, displaymode, playersize, showcontrols, autoplay, intro, introformat, completionpercent")) {
        return null;
    }

    $info = new cached_cm_info();
    $info->name = $supervideo->name;
    if ($coursemodule->showdescription && $supervideo->displaymode != "course") {
...
/**
 * Callback fired while rendering the course page: when an activity is set to "Course Page"
 * display mode, this shows the description and the media player (with the language switcher)
 * directly on the course page, instead of requiring learners to open the activity.
 *
 * @param cm_info $cm
 *
 * @return void
...
function supervideo_cm_info_view(cm_info $cm) {
    ...
    if (!$supervideo || $supervideo->displaymode != "course") {
        return;
    }
    ...
    try {
        $content = format_module_intro("supervideo", $supervideo, $cm->id, false);
        $content .= \mod_supervideo\output\language_player::render($cm, get_course($cm->course), $supervideo, $context);

        $cm->set_content($content);
    } catch (Exception $e) {
        debugging("mod_supervideo: could not render course page content for cmid {$cm->id}: " . $e->getMessage(), DEBUG_DEVELOPER);
    }
}
