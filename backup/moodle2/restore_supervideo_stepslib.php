...
class restore_supervideo_activity_structure_step extends restore_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $paths[] = new restore_path_element('supervideo_media', '/activity/supervideo/medias/media');
    ...
    /**
     * Process one restored media item.
     *
     * @param array $data parsed element data
     *
     * @throws dml_exception
     */
    protected function process_supervideo_media($data) {
        global $DB;

        $data = (object)$data;
        $oldid = $data->id;
        $data->supervideo = $this->get_new_parentid('supervideo');
        ...
        $newitemid = $DB->insert_record('supervideo_media', $data);
        $this->set_mapping('supervideo_media', $oldid, $newitemid, true);
    }
    ...
    protected function after_execute() {
        ...
        $this->add_related_files('mod_supervideo', 'media', 'supervideo_media');

        $fs = get_file_storage();
        $contextid = $this->task->get_contextid();
        $activityid = $this->task->get_activityid();
        $files = $DB->get_records("files", ["contextid" => $contextid, "component" => "mod_supervideo", "filearea" => "content"]);
